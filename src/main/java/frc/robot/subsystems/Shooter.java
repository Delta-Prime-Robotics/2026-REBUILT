...
public class Shooter extends SubsystemBase {
  ...
  static {
    ...
    m_leaderConfig.smartCurrentLimit(MotorConstants.kNeoSmartCurrentLimit)
    ...
    m_leaderConfig.closedLoop.feedForward
    .sva(
      0.0,// ks(volts)
    ...
    m_followerConfig.apply(m_leaderConfig)
    .follow(CanIdsOtherThanDrive.kShooterLeaderId, false);
  }


  /** Creates a new Shooter. */
  public Shooter() {
    m_shooterLeader = new SparkMax(CanIdsOtherThanDrive.kShooterLeaderId, SparkMax.MotorType.kBrushless);
    m_shooterFollower = new SparkMax(CanIdsOtherThanDrive.kShooterFollowerId, SparkMax.MotorType.kBrushless);

    m_Encoder = m_shooterLeader.getEncoder(); //or getAlternateEncoder()
    m_Encoder.setPosition(0);

    //To-Do: make sure this async doesnt cause issues
    m_shooterLeader.configureAsync(m_leaderConfig, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
    m_shooterFollower.configureAsync(m_followerConfig, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
  ...
  private void setShooterSetpoint(double setpointRPM){
    if (setpointRPM > kShooterMaxRPM) {
      ...
      System.out.println("WARNING: Shooter setpoint RPM exceeds maximum. Capping to " + kShooterMaxRPM + " RPM.");
    }
