...
/**
 * The Haptics subsystem controls the rumble functionality of a game controller. It provides methods
 * to set the rumble on the left, right, or both sides of the controller.
 */
public class Haptics extends SubsystemBase {
  ...
  public Command buzzLeft(double buzz) {
    return runEnd(() -> setBuzz(buzz, kHeavy), () -> stopBuzzing(kHeavy));
  }

  public Command buzzRight(double buzz) {
    return runEnd(() -> setBuzz(buzz, kLight), () -> stopBuzzing(kLight));
  }

  public Command buzzBoth(double buzz) {
    return startEnd(() -> setBuzz(buzz, kMedium), () -> stopBuzzing(kMedium));
  }

  public Command buzzFor(double buzz, RumbleType type, double time) {
    return runEnd(() -> m_controller.setRumble(type, buzz), () -> stopBuzzing(type))
        .withTimeout(time);
  }
  ...
  public Command buzzVarBothFor(double lightBuzz, double heavyBuzz, double seconds) {
    return runEnd(() -> setBuzzVarBoth(lightBuzz, heavyBuzz), () -> stopBuzzing(kMedium))
        .withTimeout(seconds);
  }
